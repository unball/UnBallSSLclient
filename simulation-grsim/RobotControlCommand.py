...
class SSLVisionClient:
    ...
    def __init__(
        self, port: int = 10006, address: str = "224.5.23.2", interface: str = ""
    ):
    ...
    def print_all_robot_info(self) -> None:
        ...
        if self.packet.HasField("detection"):
            ...
            for i, robot in enumerate(detection.robots_yellow):
                print(
                    f"-Robot(Y) ({i+1:2d}/{len(detection.robots_yellow):2d}): ", end=""
                )
                print_robot_info(robot)
...
def main():
    ...
    vision = SSLVisionClient(10006, "224.5.23.2")
        ...
        while True:
            if vision.receive():
                ...
                if vision.packet.HasField("detection"):
                    ...
                    print(
                        f"Camera ID={detection.camera_id} FRAME={detection.frame_number} T_CAPTURE={detection.t_capture:.4f}"
                    )
                    print(
                        f"SSL-Vision Processing Latency                   {(detection.t_sent - detection.t_capture) * 1000.0:7.3f}ms"
                    )
                    print(
                        f"Network Latency (assuming synched system clock) {(t_now - detection.t_sent) * 1000.0:7.3f}ms"
                    )
                    print(
                        f"Total Latency   (assuming synched system clock) {(t_now - detection.t_capture) * 1000.0:7.3f}ms"
                    )
